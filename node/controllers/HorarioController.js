...
export const getCitasTerapeutaSemana = async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            diasSemana.push({
                dia: currentDay.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' }), // Nombre del día con formato "miércoles 7 de mayo"
    ...
    try {
        const citas = await CitaModel.findAll({
            where: {
                ...
                [sequelize.where(sequelize.fn('DATE', sequelize.col('fecha')), currentDate)]: true, // Compara solo la parte de la fecha
            },
            include: [
                {
                    model: ExpedienteModel,
